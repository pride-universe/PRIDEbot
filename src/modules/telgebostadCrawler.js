const listUrl = 'https://hyresborsen.telge.se/ledigt/lagenhet';
const objUrl = 'https://hyresborsen.telge.se/ledigt/detalj/id/';
...
const fetchListings = async () => {
  ...
  const fetchData = async (url, body) => {
    ...
    if(!body) {
      ...
      result = await axios.post(url, qs.stringify(body), {headers: {'Content-Type': 'application/x-www-form-urlencoded'}, maxRedirects: 0});
    }
  ...
  function getPageStatus($) {
    const cur = Number.parseInt($('#ctl00_ctl01_DefaultSiteContentPlaceHolder1_Col1_ucNavBar_lblCurrPage').text());
    const last = Number.parseInt($('#ctl00_ctl01_DefaultSiteContentPlaceHolder1_Col1_ucNavBar_lblNoOfPages').text());
    return [cur, last];
  }
  ...
  function parsePage({ $ }, page)  {
    ...
    */
    const parseRow = (row) => {
      ...
      const ret = {};
      ret.applicants = Number.parseInt($('span', row.children.pop()).text());
      ret.moveIn = $('span', row.children.pop()).text();
      ret.rent = Number.parseInt($('span', row.children.pop()).text().replace(/\D/gi, ''));
      ...
      ret.objectId = href.match(/\/([^/]+)$/)[1];
      ret.url = `${objUrl}${ret.objectId}`;
